...
public class ColorSwap : MonoBehaviour
{
    ...
    void test(Texture2D tex)
    {
        Debug.Log($"{tex.width}:{tex.height} - " +
            $"64 ({ColorUtility.ToHtmlStringRGBA(tex.GetPixel(64, 1))}) " +
            $"128 ({ColorUtility.ToHtmlStringRGBA(tex.GetPixel(128, 1))})");
        Color[] ps = tex.GetPixels();
        string color = "";
        for (int i = 0; i < ps.Length; i++)
        {

            if (!ColorUtility.ToHtmlStringRGBA(ps[i]).Equals(color))
            {
                color = ColorUtility.ToHtmlStringRGBA(ps[i]);
                Debug.Log($"{i} : {color}");
            }
        }
    }
